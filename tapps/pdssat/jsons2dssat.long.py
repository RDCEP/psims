...
class DSSATXFileOutput:
    ...
    def __init__(self, exp_file, soil_file, use_ptransfer = True): # need soil file for initial soil conditions
    ...
    def toXFile(self):
        ...
        cu_arr2 = [] # tracks p1, p2, p5, g2, g3, and phint
                ...
                for k in range(len(evt_arr)):
                    ...
                    if self.__get_obj(evt_data, 'seqid', dblank) == seq_id:
                        ...
                        if self.__get_obj(evt_data, 'event', dblank) == 'planting':
                            # planting
                            if 'p1' in evt_data and 'p2' in evt_data and 'p5' in evt_data and \
                               'g2' in evt_data and 'g3' in evt_data and \
                               'phint' in evt_data and 'eco' in evt_data:
                                self.__copy_item(cu_data2, evt_data, 'p1')
                                self.__copy_item(cu_data2, evt_data, 'p2')
                                self.__copy_item(cu_data2, evt_data, 'p5')
                                self.__copy_item(cu_data2, evt_data, 'g2')
                                self.__copy_item(cu_data2, evt_data, 'g3')
                                self.__copy_item(cu_data2, evt_data, 'phint')
                                self.__copy_item(cu_data2, evt_data, 'eco')
                                self.__copy_item(cu_data2, evt_data, 'crid')
                                cu_num2 = self.__set_sec_data(cu_data2, cu_arr2)
                                cu_data['cul_id'] = str(cu_num2 - 1).zfill(6)
                            else:
                            ...
                            mp_data.pop('cul_id')
                            mp_data.pop('crid')
                            if 'p1' in mp_data:    mp_data.pop('p1')
                            if 'p2' in mp_data:    mp_data.pop('p2')
                            if 'p5' in mp_data:    mp_data.pop('p5')
                            if 'g2' in mp_data:    mp_data.pop('g2')
                            if 'g3' in mp_data:    mp_data.pop('g3')
                            if 'phint' in mp_data: mp_data.pop('phint')
                            if 'eco' in mp_data:   mp_data.pop('eco')
    ...
    def __write_cul_file(self, cu_arr):
        ...
        c_str = '*%s CULTIVAR COEFFICIENTS: MZCER045 MODEL\n\n' % crid2name[crid]
        c_str += '@VAR#  VRNAME.......... EXPNO   ECO#    P1    P2    P5    G2    G3 PHINT\n'
        for i in range(len(cu_arr)):
            culid = str(i).zfill(6)
            varname = 'Generic' + str(i).zfill(6)
            c_str += culid + ' ' + varname + '        . ' + cu_arr[i]['eco']
            c_str += for_str(cu_arr[i]['p1'], 0, 'r', 6, 'r', 1)
            c_str += for_str(cu_arr[i]['p2'], 0, 'r', 6, 'r', 3)
            c_str += for_str(cu_arr[i]['p5'], 0, 'r', 6, 'r', 1)
            c_str += for_str(cu_arr[i]['g2'], 0, 'r', 6, 'r', 1)
            c_str += for_str(cu_arr[i]['g3'], 0, 'r', 6, 'r', 2)
            c_str += for_str(cu_arr[i]['phint'], 0, 'r', 6, 'r', 2) + '\n'
        return c_str
...
xfileoutput = DSSATXFileOutput(options.efile, options.sfile, use_ptransfer = options.pfcn)
